...
def fbconnect():
  ...
  # First verify the value of state  to protect against cross site reference forgery attacks
  if request.args.get('state') != login_session['state']:
    response = make_response(json.dumps('Invalid state parameter.'), 401)
    response.headers['Content-Type'] = 'application/json'
    return response

  # Now we exchange this short-lived token for the long-lived token here.
  ...
  # note you have to send the app_secret along to Facebook to verify the server's identity
  url = 'https://graph.facebook.com/oauth/access_token?grant_type=fb_exchange_token&client_id=%s&client_secret=%s&fb_exchange_token=%s' \
         % (app_id, app_secret, access_token)
  h = httplib2.Http()
  # This token includes an expires field that indicates how long this token is valid. Long term
  # tokens can last up to two months. 
  result = h.request(url, 'GET')[1]

  # Use token to get user info from API
  userinfo_url = "https://graph.facebook.com/v2.2/me"
  #strip expire tag form access token since you don't need it to make API calls
  token = result.split('&')[0]

  # if token works then we should be able to make API calls with the new token as shown below in
  # the next 3 lines
  ...
  # Here we populate the login_session with the token provided.
  ...
  # Note facebook uses a separate API call to retrieve a profile picture.
  # Get user picture
...
def deleteRestaurant(restaurant_id):
  ...
  if restaurantToDelete.user_id != login_session['user_id']:
    return "<script>function myFunction() {alert('You are not authorized to delete this \
            restaurnt. Please create your own restaurant in order to delete.');}</script><body \
            onload = 'myFunction()''>"
  if request.method == 'POST':
